...
const SellerEdit = () => {
  ...
  const handleInputChange = (event: ChangeEvent<HTMLSelectElement | HTMLTextAreaElement | HTMLInputElement>) => {
  ...
  const handleMainImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    console.log("뭐가 찍히나 보자 이벤트이미지", file);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const response = await axios.patch("http://localhost:8080/seller/products/{product_id}", formData, {
        headers: {
